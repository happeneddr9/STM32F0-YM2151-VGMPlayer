...
class SFixed {
	...
	public:
		...
		static constexpr uintmax_t InternalSize = FIXED_POINTS_DETAILS::BitSize<
				InternalType>::Value;
	...
	public:
		...
		static constexpr MaskType IntegerMask =
				FIXED_POINTS_DETAILS::IdentityMask<IntegerSize>::Value;
		static constexpr MaskType FractionMask =
				FIXED_POINTS_DETAILS::IdentityMask<FractionSize>::Value;

		static constexpr MaskType IdentityMask = (IntegerMask << IntegerShift)
				| (FractionMask << FractionShift);

		static constexpr MaskType MidpointMask = FIXED_POINTS_DETAILS::MsbMask<
				FractionSize>::Value;
	...
	protected:
		class RawType {
			...
			public:
				constexpr inline explicit RawType(const InternalType & value) :
						value(value) {
				}
				constexpr inline explicit operator InternalType() const {
					return this->value;
				}
		};
	...
	public:
		...
		static constexpr SFixed MinValue = SFixed::fromInternal(
				FIXED_POINTS_DETAILS::MsbMask<InternalSize>::Value);
		static constexpr SFixed MaxValue = SFixed::fromInternal(
				~FIXED_POINTS_DETAILS::MsbMask<InternalSize>::Value);
...
template<unsigned Integer, unsigned Fraction>
constexpr SFixed<Integer * 2, Fraction * 2> multiply(
		const SFixed<Integer, Fraction> & left,
		const SFixed<Integer, Fraction> & right);
...
template<unsigned Integer, unsigned Fraction>
constexpr bool operator ==(const SFixed<Integer, Fraction> & left,
		const SFixed<Integer, Fraction> & right);
...
template<unsigned IntegerLeft, unsigned FractionLeft, unsigned IntegerRight,
		unsigned FractionRight>
constexpr bool operator ==(const SFixed<IntegerLeft, FractionLeft> & left,
		const SFixed<IntegerRight, FractionRight> & right);
...
template<unsigned Integer, unsigned Fraction>
constexpr SFixed<Integer, Fraction> operator +(
		const SFixed<Integer, Fraction> & left,
		const SFixed<Integer, Fraction> & right);
...
template<unsigned IntegerLeft, unsigned FractionLeft, unsigned IntegerRight,
		unsigned FractionRight>
constexpr auto operator +(const SFixed<IntegerLeft, FractionLeft> & left,
		const SFixed<IntegerRight, FractionRight> & right)
...
template<unsigned IntegerLeft, unsigned FractionLeft, unsigned IntegerRight,
		unsigned FractionRight>
inline constexpr auto operator /(const SFixed<IntegerLeft, FractionLeft> & left,
		const SFixed<IntegerRight, FractionRight> & right)
		-> FIXED_POINTS_DETAILS::LargerType< SFixed<IntegerLeft, FractionLeft>, SFixed<IntegerRight, FractionRight> >;
