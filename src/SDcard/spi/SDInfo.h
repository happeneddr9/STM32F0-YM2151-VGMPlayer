/**
 * @Name    : 	SDInfo.h
 * @Date    : 	2019年12月29日 下午4:12:48
 * @Author  : 	happe
 * @brief   : 	here defined all of the SdFat library needed commands, error code, register,
 * 				card types, and also defined Card Capacity calculate code.
 *
 */

#ifndef SDCARD_SPI_SDINFO_H_
#define SDCARD_SPI_SDINFO_H_

namespace SDCard {
	//------------------------------------------------------------------------------
	// SD operation timeouts
	/** CMD0 retry count */
	const uint8_t SD_CMD0_RETRY = 10;			// 10
	/** command timeout ms */
	const uint16_t SD_CMD_TIMEOUT = 200;		// 200
	/** init timeout ms */
	const uint16_t SD_INIT_TIMEOUT = 2000;		// 2000
	/** erase timeout ms */
	const uint16_t SD_ERASE_TIMEOUT = 10000;
	/** read timeout ms */
	const uint16_t SD_READ_TIMEOUT = 1000;		// 1000
	/** write time out ms */
	const uint16_t SD_WRITE_TIMEOUT = 2000;		// 2000
	//------------------------------------------------------------------------------
	/* SD card commands */
	/** GO_IDLE_STATE - init card in spi mode if CS low */
	const uint8_t CMD0 = 0X00;

	const uint8_t CMD1 = 0X01;
	...
	enum CS_RELEASE : bool {
		RELEASE = true,			// Release CS
		NO_RELEASE = false,
	};
