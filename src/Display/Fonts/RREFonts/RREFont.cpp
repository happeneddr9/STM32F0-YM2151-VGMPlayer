...
void RREFont::init(FillRectFunPtr rectFun, int16_t swd,
		int16_t sht) {
	fillRectFun = rectFun;
	scrWd = swd;
	scrHt = sht;
...
// ---------------------------------
int16_t RREFont::charWidthNoSort(uint8_t c, int16_t *_xmin) {
	if (c < rFont->firstCh || c > rFont->lastCh)
		return c == ' ' ? rFont->wd / 2 : 0;
	uint16_t recIdx = pgm_read_word(&(rFont->offs[c - rFont->firstCh]));
	uint16_t recNum = pgm_read_word(&(rFont->offs[c - rFont->firstCh + 1]))
			- recIdx;
	if (!recNum)
		return rFont->wd / 2;
	int16_t i, xmin = rFont->wd, xmax = 0;
	switch (rFont->type & 7) {
		case RRE_16B:
			for (i = 0; i < recNum; i++) {
				uint16_t *rects = (uint16_t*) rFont->rects;
				int16_t v = pgm_read_word(&rects[i + recIdx]);
				xf = v & 0x000f;
				wf = 1 + ((v & 0x0f00) >> 8);
				if (xf < xmin)
					xmin = xf;
...
// ----------------------------------------------------------------
int16_t RREFont::charWidthOptim(uint8_t c, int16_t *_xmin) {
	if (c < rFont->firstCh || c > rFont->lastCh)
		return c == ' ' ? rFont->wd / 2 : 0;
	uint16_t recIdx = pgm_read_word(&(rFont->offs[c - rFont->firstCh]));
	uint16_t recNum = pgm_read_word(&(rFont->offs[c - rFont->firstCh + 1]))
...
#if ENABLE_RRE_24B==1
		case RRE_24B: {
			uint8_t *rects = (uint8_t*) rFont->rects
					+ (recNum - 1 + recIdx) * 3;
			chWd = (pgm_read_byte(&rects[0]) & 0x3f)
					+ (pgm_read_byte(&rects[2]) & 0x3f) + 1;
		}
			break;
#endif
#if ENABLE_RRE_32B==1
...
// ----------------------------------------------------------------
int16_t RREFont::charWidth(uint8_t c, int16_t *_xmin) {
#if CONVERT_PL_CHARS==1
	c = convertPolish(c);
#endif
#if ENABLE_NOSORT==1
	int16_t wd = (rFont->type & RRE_NO_SORT) ? charWidthNoSort(c,_xmin) : charWidthOptim(c,_xmin);
#else
	int16_t wd = charWidthOptim(c, _xmin);
#endif
	int16_t wdL = 0, wdR = spacing; // default spacing before and behind char
...
// ----------------------------------------------------------------
int16_t RREFont::drawChar(int16_t x, int16_t y, unsigned char c) {
#if CONVERT_PL_CHARS==1
	c = convertPolish(c);
#endif
	if (x >= scrWd || y >= scrHt || x + rFont->wd * sx - 1 < 0
			|| y + rFont->ht * sy - 1 < 0)
		return 0;
	if (c < rFont->firstCh || c > rFont->lastCh)
		return charWidth(c);
	...
	uint16_t recNum = pgm_read_word(&(rFont->offs[c - rFont->firstCh + 1]))
			- recIdx;
#if ENABLE_NOSORT==1
	int16_t xmin, chWd = (rFont->type & RRE_NO_SORT) ? charWidthNoSort(c,&xmin) : charWidthOptim(c,&xmin);
#else
	int16_t xmin, chWd = charWidthOptim(c, &xmin);
#endif
	int16_t wd = chWd, wdL = 0, wdR = spacing;
	if ((*isNumberFun)(c) && minDigitWd > 0) {
		if (minDigitWd > wd) {
			wdL = (minDigitWd - wd) / 2;
			wdR += (minDigitWd - wd - wdL);
	...
	if (x + wd + wdL + wdR > scrWd)
		wdR = max(scrWd - x - wdL - wd, 0);
	if (x + wd + wdL + wdR > scrWd)
		wd = max(scrWd - x - wdL, 0);
	if (x + wd + wdL + wdR > scrWd)
		wdL = max(scrWd - x, 0);
	wd += wdR + wdL;
	int16_t type = rFont->type & 7;
	if (bg != fg
			&& (type == RRE_16B || type == RRE_24B || type == RRE_32B || !recNum))
	...
	switch (type) {
		...
		case RRE_16B:
			for (int16_t i = 0; i < recNum; i++) {
				...
				wf = ((v & 0x0f00) >> 8) + 1;
				hf = ((v & 0xf000) >> 12) + 1;
				(*fillRectFun)(x + xf * sx, y + yf * sy, bold + wf * sx,
						hf * sy, fg);
		...
		case RRE_24B:
			for (int16_t i = 0; i < recNum; i++) {
				...
				hf = 1
						+ (((pgm_read_byte(&rects[0]) & 0xc0) >> 6)
								| ((pgm_read_byte(&rects[1]) & 0xc0) >> 4)
								| ((pgm_read_byte(&rects[2]) & 0xc0) >> 2));
				(*fillRectFun)(x + xf * sx, y + yf * sy, bold + wf * sx,
						hf * sy, fg);
			}
			break;
		...
		case RRE_V16B:
			if (bg != fg) {
				if (wdL > 0)
					(*fillRectFun)(x - wdL * sx, y, wdL * sx, rFont->ht * sy,
							bg);
				...
				while (idx < recNum) {
					...
					xf = (v & 0x3f) - xmin;
					int16_t ybg = 0, xfCur = xf;
					while (xf == xfCur && idx < recNum) {
						v = pgm_read_word(rects + idx);
						yf = (v >> 6) & 0x1f;
						hf = ((v >> 11) & 0x1f) + 1;
						if (yf > ybg)
							(*fillRectFun)(x + xfCur * sx, y + ybg * sy, 1 * sx,
									(yf - ybg) * sy, bg);
						ybg = yf + hf;
						(*fillRectFun)(x + xfCur * sx, y + yf * sy,
								bold + 1 * sx, hf * sy, fg);
						idx++;
						v = pgm_read_word(rects + idx);
						xf = (v & 0x3f) - xmin;
					...
					// last bg line
					if (ybg < rFont->ht)
						(*fillRectFun)(x + xfCur * sx, y + ybg * sy,
								bold + 1 * sx, (rFont->ht - ybg) * sy, bg);
				}
				if (wdR > 0)
					(*fillRectFun)(x + chWd * sx, y, wdR * sx, rFont->ht * sy,
							bg);
			} else
				for (int16_t i = 0; i < recNum; i++) {
					uint16_t *rects = (uint16_t*) rFont->rects;
			...
			if (bg != fg) {
				if (wdL > 0)
					(*fillRectFun)(x - wdL * sx, y, wdL * sx, rFont->ht * sy,
							bg);
				uint16_t *rects = (uint16_t*) rFont->rects;
				int16_t idx = 0, yfCur = 0;
				while (idx < recNum) {
					uint16_t v = pgm_read_word(rects + idx + recIdx);
					yf = (v >> 5) & 0x3f;
					if (yf - yfCur > 1 /*|| (yf-yfCur>0 && yfCur==0)*/)
						(*fillRectFun)(x + 0 * sx, y + yfCur * sy, chWd * sx,
								(yf - yfCur) * sy, bg);
					...
					// last bg line
					if (xbg < chWd)
						(*fillRectFun)(x + xbg * sx, y + yfCur * sy,
								(chWd - xbg) * sx, 1 * sy, bg);
					//Serial.print(idx); Serial.print(" ++ "); Serial.print(yf); Serial.print("  "); Serial.println(yfCur);
					if (idx == recNum - 1 && yfCur < rFont->ht)
						(*fillRectFun)(x + 0 * sx, y + yfCur * sy, chWd * sx,
								(yfCur < rFont->ht - yfCur) * sy, bg);
				}
				if (wdR > 0)
					(*fillRectFun)(x + chWd * sx, y, wdR * sx, rFont->ht * sy,
							bg);
...
// ----------------------------------------------------------------
int16_t RREFont::strWidth(const char *str) {
	int16_t wd = 0;
	while (*str)
		wd += charWidth(*str++);
	return wd;
...
// ----------------------------------------------------------------
int16_t RREFont::printStr(int16_t xpos, int16_t ypos, const char *str) {
	//unsigned char ch;
	//int16_t stl, row;
	int16_t x = xpos;
	int16_t y = ypos;
	int16_t wd = strWidth(str);

	if (x == ALIGN_RIGHT)
		x = scrWd - wd; // right = -1
	else if (x < 0)
		x = (scrWd - wd) / 2; // center = -2
	if (x < 0)
		x = 0; // left

	while (*str) {
		char ch = *str++;
		int16_t wd = drawChar(x, y, ch);
		x += wd;
		if ((cr && x >= scrWd) || ch == 10) {
			x = cr ? 0 : xpos;
