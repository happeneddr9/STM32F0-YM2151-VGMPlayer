...
Adafruit_GFX::Adafruit_GFX(int16_t w, int16_t h)
	...
	textcolor = textbgcolor = Pixel::White;
	wrap = true;
	_cp437 = false;
	gfxFont = nullptr;
}
...
void Adafruit_GFX::drawCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, uint16_t color) {
	...
	while (x < y) {
		...
		if (cornername & 0x2) {						// Upper Right corner
			writePixel(x0 + x, y0 - y, color);
			writePixel(x0 + y, y0 - x, color);
		}
		...
		if (cornername & 0x1) {						// Upper Left corner
			writePixel(x0 - y, y0 - x, color);
			writePixel(x0 - x, y0 - y, color);
		}
	}
 ...
 */
/**************************************************************************/
void Adafruit_GFX::fillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t corners, int16_t delta,
		uint16_t color) {
 ...
 ***************************************************************************************/
void Adafruit_GFX::drawEllipse(int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color) {
	if (rx < 2)
		return;
	if (ry < 2)
		return;
 ...
 ***************************************************************************************/
void Adafruit_GFX::fillEllipse(int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color) {
	if (rx < 2)
		return;
	if (ry < 2)
		return;
	...
	startWrite();
	for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
		writeFastHLine(x0 - x, y0 - y, x + x + 1, color);
		writeFastHLine(x0 - x, y0 + y, x + x + 1, color);
 ...
 */
/**************************************************************************/
void Adafruit_GFX::drawGrayscaleBitmap(int16_t x, int16_t y, const uint8_t bitmap[], const uint8_t mask[],
		int16_t w, int16_t h) {
 ...
 */
/**************************************************************************/
void Adafruit_GFX::drawGrayscaleBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint8_t *mask, int16_t w,
		int16_t h) {
	int16_t bw = (w + 7) / 8; // Bitmask scanline pad = whole byte
	uint8_t byte = 0;
	startWrite();
...
void Adafruit_GFX::drawSRGBBitmap(int16_t x, int16_t y, const uint16_t bitmap[], int16_t w, int16_t h, size_t size){
	if(size <= 1)
		return drawRGBBitmap(x, y, bitmap, w, h);



	startWrite();
	for (int16_t j = 0; j < h; j++, y += size) {
		int16_t xpos = 0;
		for (int16_t i = 0; i < w; i++, xpos += size) {
			writeFillRect(x + xpos, y, size, size, pgm_read_word(&bitmap[j * w + i]));
		}
	}
	endWrite();
}

void Adafruit_GFX::drawSRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h, size_t size){
	if(size <= 1)
		return drawRGBBitmap(x, y, bitmap, w, h);
	...
	for (int16_t j = 0; j < h; j++, y += size) {
		int16_t xpos = 0;
		for (int16_t i = 0; i < w; i++, xpos += size) {
			writeFillRect(x + xpos, y, size, size, bitmap[j * w + i]);
		}
	}
	endWrite();
}
 ...
 */
/**************************************************************************/
void Adafruit_GFX::drawRGBBitmap(int16_t x, int16_t y, const uint16_t bitmap[], const uint8_t mask[],
		int16_t w, int16_t h) {
	int16_t bw = (w + 7) / 8; // Bitmask scanline pad = whole byte
	uint8_t byte = 0;
	startWrite();
...
void Adafruit_GFX::drawChar(int16_t x, int16_t y, unsigned char c,
	...
	if (!gfxFont) { // 'Classic' built-in font
		...
		for (int8_t i = 0; i < 5; i++) { // Char bitmap = 5 columns
			uint8_t line = pgm_read_byte(&Standard_ASCII_5x7_font[c * 5 + i]);
